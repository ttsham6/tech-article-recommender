...
def main() -> None:
    """
    CLI引数を解釈し、feedparser で RSS 1本を KB 用の記事ファイル群へ変換する。
    """
...
def write_article(entry: feedparser.FeedParserDict, output_dir: Path, source_label: str, index: int) -> Path:
    """
    ...
    article_path.with_name(article_path.name + ".metadata.json").write_text(
